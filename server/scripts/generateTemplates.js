...
const templateNames = [
  'welcome', 'forgotPassword', 'appointmentBooked', 'appointmentConfirmed', 
  'appointmentCancelled', 'appointmentReminder', 'paymentSuccess', 
  'paymentReceipt', 'prescriptionUploaded', 'healthRecordUpdated'
];

templateNames.forEach(name => {
  const subjectStr = name.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
  const content = `module.exports = (data) => {
  return {
    subject: '${subjectStr} - MediBook',
    html: \`
